...
阶段 3.5 仅接受 `Level = 0`；阶段 4 起允许任意非负 Level，但起点、终点必须在同一
Level。矩形与传送带路径的区别固定为：
...
完成状态（2026-08-15）：已实现 Flat 多层 Surface 验证、同 X/Z 多层独立 Occupancy、
地基任意非负层自由放置与允许拆除下层地基、8 层 Physics LevelBand、实际 Foundation Collider 首命中拾取、
`Y = 0` 空地回退、建筑/端口/预览/运输物品层高换算、选层 HUD 和按 Revision 更新的区块
可见性。自动化回归为 EditMode 161/161、PlayMode 10/10 通过；跨层运输连接仍按计划留在
阶段 5 及后续坡道/垂直结构阶段。
...
- `Level = 0` 的地基顶面对应世界坐标 `Y = 0`，空场景不依赖已有 Surface 或地形 Collider；
- `Level > 0` 的地基不要求同一 X/Z、`Level - 1` 已有地基支撑；
- 矩形首点确定后，预览和第二点拾取固定投射到首点平面，命令中的所有格必须位于同一 Level；
- 命中已有地基的朝上表面并选择地基建造时，目标格为命中地基的 `Level + 1`。
...
- 从空地创建第一块地基时，明确回退到世界 `Y = 0` 建造工作平面，不要求场景中已有地形或地基；
...
完成状态（2026-08-15）：已实现包含 `Level` 的运输节点索引、同层平面自动边和按 Entity
端点注册的 `TransportExplicitEdge`。显式边在拓扑重建时优先编译为连续 NodeIndex，Fixed Tick
不执行几何查询；重建仅由 `TransportTopologyRevision` 驱动，并新增 Transport Edge Count 诊断。
跨层显式链的前移、堵尾反压、满环原子移动和显式边优先级已纳入自动化回归。EditMode
全量回归为 EditMode 169/169、PlayMode 10/10 通过。阶段 6、7 只需生成特殊节点链和
显式边，不改变现有仲裁核心。
...
- 垂直带由首尾接口节点和按长度展开的中间运输节点组成；去除首尾后，每个中间节点与
  普通平面传送带相同，保持单节点单物品、独立进度和逐节点阻塞语义。
- 垂直带向运输拓扑注册“入口 → 中间节点链 → 出口”的有向显式边，而不是把上下端直接
  连成一条无状态边。
...
1. `[已定稿]` 地基可在任意非负层悬空放置，也允许拆除下层地基，不做承重拒拆。
2. `[已定稿]` 拆除承载普通建筑的地基时拒绝，不级联拆除建筑。
3. 坡道是否允许跨越普通建筑体积，以及其碰撞包围盒规则。
4. 坡道传送带是否按每个 Ramp Slot 收费和拆除。
5. `[部分定稿]` 垂直带必须包含按长度展开的中间运输节点，阻塞和传输语义与平面带一致；
   带内有物品时的拆除规则仍待确定。
