...
- [x] 所有传送、合流、分流、环路和端口回归测试通过。
- [x] 稳态 Tick 无托管 GC 分配。
- [x] Resolver 为近似 `O(N)`，且不在每 Tick 重建拓扑。
- [x] Transfer 主路径由 Burst Native Job 执行。
- [x] 逻辑状态提交不再逐 Entity 通过主线程 EntityManager 完成。
- [x] Grid 和建筑 Transform 只在状态变化时更新。
- [x] 物品视觉每渲染帧最多更新一次。
- [x] Item 创建销毁不会随稳定吞吐量持续产生结构变化。
- [x] 4096 节点压力场景达到目标帧预算且结果确定。
- [x] Profiler 基线、优化后数据和测试场景一并保留。
